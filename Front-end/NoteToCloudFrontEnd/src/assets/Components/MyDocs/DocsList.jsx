...
function DocsList() {
    ...
    const [listReadyFiles, setListReadyFiles] = useState([]);
    //État list des documents pas prêt à être consultés
    const [listNotReadyFiles, setListNotReadyFiles] = useState([])
    ...
    function openPDF(d) {
        axios.post("http://127.0.0.1:5000/getPDF", { "id": localStorage.getItem("user"), "file": listReadyFiles[d] + ".pdf" }, { responseType: "blob" })
            .then(function (response) {
    ...
    function openTXT(d) {
        axios.post("http://127.0.0.1:5000/getTXT", { "id": localStorage.getItem("user"), "file": listReadyFiles[d] + ".pdf" }, { responseType: "blob" })
            .then(function (response) {
                if (response.data.size === 4) {
                    console.log("No documents")
                } else {
        ...
        axios.post("http://127.0.0.1:5000/getTXT", { "id": localStorage.getItem("user"), "file": listReadyFiles[d] + ".pdf" }, { responseType: "blob" })
            .then(function (response) {
                if (response.data.size === 4) {
                    ...
                    const blob = new Blob([response.data], { type: 'text/plain' });
                    downloadBlob(blob, listReadyFiles[d] + ".txt")
                }
            })
    }
    ...
    function deleteFile(fileName) {
        if (window.confirm("Do you really want to delete this file?")) {
            axios.post("http://127.0.0.1:5000/delete", { "id": localStorage.getItem("user"), "file": fileName })
                .then(function (response) {
                    ...
                    <Tooltip.Root delayDuration={0}>
                        ...
                        <Tooltip.Portal className='portal'>
                            <Tooltip.Content className="TooltipContent" sideOffset={5} >
                                <button id='download' onClick={() => { deleteFile(d) }}>Delete</button>
                            </Tooltip.Content>
                        </Tooltip.Portal>
                    </Tooltip.Root>
                </Tooltip.Provider>
            </div>
            <button id='download' onClick={openPDF.bind(this, index)}>PDF</button>
            <div>
                <Tooltip.Provider >
                    <Tooltip.Root delayDuration={0}>
                        <Tooltip.Trigger asChild>
                        ...
                        <Tooltip.Portal className='portal'>
                            <Tooltip.Content className="TooltipContent" sideOffset={5} >
                                <button id='download' onClick={openTXT.bind(this, index)}>View</button>
                                <button id='download' onClick={downloadTXT.bind(this, index)}>Download</button>
                            </Tooltip.Content>
                        </Tooltip.Portal>
                    </Tooltip.Root>
                </Tooltip.Provider>
    ...
    const listOfNotReadyDocs = listNotReadyFiles.map((d) =>
        <li key={d}>
            {d}<Spinner></Spinner>
        </li>
    )
